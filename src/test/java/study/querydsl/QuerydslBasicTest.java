...
public class QuerydslBasicTest {
    ...
    */
    ...
    private List<Member> searchMember2(String usernameParam, Integer ageParam) {
        return jpaQueryFactory.selectFrom(member)
                .where(userEq(usernameParam),ageEq(ageParam))
//              .where(allEq(usernameParam,ageParam)
                .fetch();
    }

    private BooleanExpression userEq(String usernameParam) {
    ...
    public void sqlFunction() {
        List<String> result = jpaQueryFactory.select(Expressions.stringTemplate(
...
//                .where(member.username.eq(member.username.lower())) 다음과 같이 querydsl 에서 지원해주는 것도 있음.
                .fetch();
        for (String string : result) {
            System.out.println("string = " + string);
        }
